...
async function postBalancedTransaction({ reference, type, description, userAccountId, amountKobo, direction, counterType, counterName, metadata }) {
  if (!Number.isSafeInteger(amountKobo) || amountKobo <= 0) throw new ApiError(400, 'INVALID_AMOUNT', 'Amount must be a positive integer in kobo.');
  return prisma.$transaction(async tx => {
    const prior = await tx.ledgerTransaction.findUnique({ where: { reference }, include: { postings: true } });
    if (prior) return prior;
    const userDelta = direction === 'credit' ? amountKobo : -amountKobo;
    if (userDelta < 0) {
      const updated = await tx.ledgerAccount.updateMany({ where: { id: userAccountId, balanceKobo: { gte: amountKobo } }, data: { balanceKobo: { decrement: amountKobo } } });
      if (updated.count !== 1) throw new ApiError(409, 'INSUFFICIENT_BALANCE', 'Your wallet balance is too low.');
    } else {
      await tx.ledgerAccount.update({ where: { id: userAccountId }, data: { balanceKobo: { increment: amountKobo } } });
    }
